        ...
        switch (iType) {
            ...
            case 4:
                if (iNumValues == 1) return oFile.getLongAt(iEntryOffset + 8, bBigEnd);
                    ...
                    var aVals = [];
                    for (var n = 0; n < iNumValues; n++) aVals[n] = oFile.getLongAt(iValueOffset + 4 * n, bBigEnd);
                    return aVals
                }
                break;
            case 5:
                if (iNumValues == 1) return oFile.getLongAt(iValueOffset, bBigEnd) / oFile.getLongAt(iValueOffset + 4, bBigEnd);
                else {
                    var aVals = [];
                    for (var n = 0; n < iNumValues; n++) aVals[n] = oFile.getLongAt(iValueOffset + 8 * n, bBigEnd) / oFile.getLongAt(iValueOffset + 4 + 8 * n, bBigEnd);
                    return aVals
                }
                break;
            case 9:
                if (iNumValues == 1) return oFile.getSLongAt(iEntryOffset + 8, bBigEnd);
                    ...
                    var aVals = [];
                    for (var n = 0; n < iNumValues; n++) aVals[n] = oFile.getSLongAt(iValueOffset + 4 * n, bBigEnd);
                    return aVals
                }
                break;
            case 10:
                if (iNumValues == 1) return oFile.getSLongAt(iValueOffset, bBigEnd) / oFile.getSLongAt(iValueOffset + 4, bBigEnd);
                else {
                    var aVals = [];
                    for (var n = 0; n < iNumValues; n++) aVals[n] = oFile.getSLongAt(iValueOffset + 8 * n, bBigEnd) / oFile.getSLongAt(iValueOffset + 4 + 8 * n, bBigEnd);
                    return aVals
                }
                break
        }
    ...
    EXIF.pretty = function(oImg) {
        ...
        for (var a in oData)
            if (oData.hasOwnProperty(a))
                if (typeof oData[a] == "object") strPretty += a + " : [" + oData[a].length + " values]\r\n";
    ...
    function loadAllImages() {
        ...
        for (var i = 0; i < aImages.length; i++)
            if (aImages[i].getAttribute("exif") == "true")
                if (!aImages[i].complete) addEvent(aImages[i], "load", function() {
                    EXIF.getData(this)
                });
...
var BinaryFile = function(strData, iDataOffset, iDataLength) {
    ...
    this.getStringAt = function(iOffset, iLength) {
        var aStr = [];
        for (var i = iOffset, j = 0; i < iOffset + iLength; i++, j++) aStr[j] = String.fromCharCode(this.getByteAt(i));
        return aStr.join("")
    };
...
(function() {
    ...
    function readInfoFromData(data) {
        var offset = 0;
        if (data.getByteAt(0) == 255 && data.getByteAt(1) == 216) return readJPEGInfo(data);
        if (data.getByteAt(0) == 137 && data.getStringAt(1, 3) == "PNG") return readPNGInfo(data);
        if (data.getStringAt(0, 3) == "GIF") return readGIFInfo(data);
        if (data.getByteAt(0) == 66 && data.getByteAt(1) == 77) return readBMPInfo(data);
        return {
            format: "UNKNOWN"
        }
    }
    ...
    function readJPEGInfo(data) {
        ...
        var exif = {};
        if (typeof EXIF != "undefined" && EXIF.readFromBinaryFile) exif = EXIF.readFromBinaryFile(data);
        return {
            format: "JPEG",
            version: "",
            width: w,
    ...
    ImageInfo.loadInfo = function(args, cb) {
        if (!files[args.filename]) readFileData(args.filename, args.binary, args.size, cb);
        else if (cb) cb()
    };
